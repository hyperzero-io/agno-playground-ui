          ...
          onChunk: (chunk: RunResponse) => {
            if (chunk.event === RunEvent.RunStarted) {
              ...
              setMessages((prevMessages) => {
                ...
                const lastMessage = newMessages[newMessages.length - 1]
                if (lastMessage && lastMessage.role === 'agent') {
                  const newToolCalls = [...(lastMessage.tool_calls || [])]
                  if (chunk.tools && chunk.tools.length > 0) {
                    chunk.tools.forEach((tool) => {
                      if (!newToolCalls.find((t) => t.tool_call_id === tool.tool_call_id)) {
                        newToolCalls.push(tool)
                      }
                    })
                  }
              ...
              setMessages((prevMessages) => {
                ...
                if (lastMessage && lastMessage.role === 'agent') {
                  ...
                  if (chunk.tools && chunk.tools.length > 0) {
                    chunk.tools.forEach((tool) => {
                      const idx = newToolCalls.findIndex((t) => t.tool_call_id === tool.tool_call_id)
                      if (idx !== -1) {
                        newToolCalls[idx] = tool
                      } else {
                        newToolCalls.push(tool)
                      }
                    })
              ...
              setMessages((prevMessages) => {
                ...
                if (
                  ...
                  if (chunk.extra_data?.reasoning_steps) {
                    if (!lastMessage.extra_data) lastMessage.extra_data = {}
                    const prevSteps = lastMessage.extra_data.reasoning_steps || []
                    const newSteps = chunk.extra_data.reasoning_steps.filter(
                      (step) => !prevSteps.some((s) => s.title === step.title && s.action === step.action)
                    )
